...
def hyper_HPO(seed):
    ...
    n_iter_search = 10 #number of iterations is set to 20, you can increase this number if time permits
    
    clf = RandomForestClassifier(random_state=seed)
    hyper = HyperbandSearchCV(clf, param_distributions =rf_params,cv=3,min_iter=10,max_iter=100,scoring='accuracy',random_state=seed,verbose=2,return_train_score=True)
    hyper.fit(X=X, y=y)
    # print(Random.best_params_)
    # print("Accuracy:"+ str(Random.best_score_))
    # print("\n每次迭代的准确率(平均交叉验证得分):")
    results = hyper.cv_results_
    best_acc = 0
    t1 = time.time()
...
if __name__ == "__main__":
    ...
    Exp_marker = "CAMO-HPO"
    # Set up logging
    log_file_path = os.path.join('/home/fillip/桌面/Hyperparameter-Optimization-of-Machine-Learning-Algorithms/Exp_res', Exp_marker, 'Hyper_experiment.log')
