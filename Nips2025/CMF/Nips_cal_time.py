...
def paint_dmf_query(axs):
    ...
    UCB_list = ['AR_UCB', 'ResGP_UCB', 'GP_UCB', 'CMF_CAR_UCB', 
                # 'CMF_CAR_dkl_UCB'
                ]
        ...
        for methods_name in acq_list:
            for data_name in cmf_data_name_list:
                for seed in seed_dic[data_name]:
                    ...
                    if methods_name in ['CMF_CAR_UCB', 'CMF_CAR_cfKG']:
...
def paint_dmf_training(axs):
    ...
    dmf_methods_list = ['AR_UCB', 'ResGP_UCB', 'GP_UCB', 'DNN_MFBO', 'DMF_CAR_UCB']
    colors = [Dic[i][0] for i in dmf_methods_list]
    # labels = ['AR', 'ResGP', 'GP', 'DNN-MFBO', 'CAMO', 'CAMO-DKL']
    labels = ['AR', 'ResGP', 'GP', 'DNN-MFBO', 'CAMO']
...
def paint_cmf_training(axs):
    ...
    dmf_methods_list = ['GP_UCB',  'fabolas', 'smac', 'CMF_CAR_UCB', 
                        # 'CMF_CAR_dkl_UCB'
                        ]
    colors = [Dic[i][0] for i in dmf_methods_list]
    labels = ['GP', 'Fabolas', 'SMAC3', 'CAMO', 
            #   'CAMO-DKL'
              ]
...
plt.tight_layout()
plt.savefig(os.path.join(sys.path[-1], 'Nips2025', 'CMF','Graphs') + '/query_time_heng' + '.png',
            bbox_inches='tight')
