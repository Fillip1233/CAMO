...
def paint_dmf_query(axs):
    ...
    cfKG_list = ['AR_cfKG', 'ResGP_cfKG', 'GP_cfKG', 'CMF_CAR_cfKG', 'CMF_CAR_dkl_cfKG']
...
def paint_dmf_training(axs):
    ...
    labels = ['AR', 'ResGP', 'GP', 'CAMO', 'CAMO-DKL', 'DNN_MFBO']
...
fig, axs = plt.subplots(2, 2, figsize=(30, 27))
paint_dmf_query(axs[0, 0])
paint_cmf_query(axs[0, 1])

paint_dmf_training(axs[1, 0])
paint_cmf_training(axs[1, 1])

# plt.tight_layout()
plt.savefig(os.path.join(sys.path[-1], 'Experiment', 'paper_yx') + '/query_time' + '.eps',
            bbox_inches='tight')
