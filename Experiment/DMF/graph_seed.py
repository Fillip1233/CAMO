...
for seed in [0,1,2,3,4]:
    ...
    for methods_name in cmf_methods_name_list:
        ...
        mean = np.mean(tem, axis=0)
        var = np.std(tem, axis=0)
        ll = plt.plot(ct[0].flatten(), mean.flatten()+ add_dict[data_name], ls='dashed', color=Dic[methods_name+'_con'][0],
    ...
    plt.savefig(os.path.join(sys.path[-1], 'Experiment', 'DMF', 'paper_seed') + '/' +'DMF_'+ data_name +'_'+ cost_name +'_seed'+str(seed)+'_.pdf', bbox_inches='tight')
    plt.savefig(os.path.join(sys.path[-1], 'Experiment', 'DMF', 'paper_seed') + '/' +'DMF_'+ data_name +'_'+ cost_name +'_seed'+str(seed)+'_.eps', bbox_inches='tight')
