...
for methods_name in methods_name_list:
    ...
    for seed in [0,1,2,3,4]:
        ...
        incumbents = data['IR'].to_numpy().reshape(-1, 1)
    ...
    plt.ylabel("Inference regret", fontsize=20)
...
seed = 'all'
plt.tight_layout()
plt.savefig(os.path.join(sys.path[-1], 'Experiment', 'DMF', 'Graphs') + '/' + data_name+'_'+cost_name +'_IR_' +'_seed_' + str(seed) + '.png',
            bbox_inches='tight')
