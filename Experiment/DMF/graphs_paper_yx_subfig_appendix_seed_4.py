...
def draw_seed(axs, seed, data_name):
    for methods_name in methods_name_list:
        ...
        ll = axs.plot(cost, SR + add_dict[data_name], ls=Dic[methods_name][-1], color=Dic[methods_name][0],
    ...
    for methods_name in cmf_methods_name_list:
        ...
        # mean = np.mean(SR_new, axis=0)
        # var = np.std(SR_new, axis=0)
        ll = axs.plot(cost, SR + add_dict[data_name], ls=Dic[methods_name+'_con'][-1], color=Dic[methods_name+'_con'][0],
                    label=Dic[methods_name+'_con'][2],
                    marker=Dic[methods_name+'_con'][1], markersize=12)
        # axs.fill_between(cost_x,
        #                 mean + add_dict[data_name] - 0.96 * var,
        #                 mean + add_dict[data_name] + 0.96 * var,
...
draw_seed(axs[0, 0], 0, data_name)
draw_seed(axs[0, 1], 1, data_name)
draw_seed(axs[1, 0], 2, data_name)
draw_seed(axs[1, 1], 3, data_name)
...
plt.tight_layout()
plt.savefig(os.path.join(sys.path[-1], 'Experiment', 'DMF', 'paper_appendix') + '/' +data_name +'_seed.pdf', bbox_inches='tight')