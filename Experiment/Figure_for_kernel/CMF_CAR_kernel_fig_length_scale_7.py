...
record = []
length_scale = torch.linspace(6, 10, 20)
...
plt.figure()
color_dic = [ '#8A2BE2', '#008B8B', '#B8860B', '#A9A9A9', '#006400', '#BDB76B', '#8B008B', '#556B2F', '#FF8C00', '#9932CC', '#8B0000', '#E9967A', '#8FBC8F', '#483D8B', '#2F4F4F', '#00CED1', '#9400D3', '#FF1493', '#0ΛBFFFI','#696969', '#1E90FFI']
...
plt.legend()
# plt.show()
plt.savefig('Kernel_compare_7-1.png') 

# plt.figure()
# for i in range(5, 11):
#     plt.plot(length_scale.numpy().flatten(), rec[:, i].flatten(), color = color_dic[i], ls = 'dashed', label = 'length_scale =' + str(length_scale[i].item())[:4])

# plt.xlabel("t")
# plt.ylabel("Cov(t, t)")
# plt.legend()
# # plt.show()
# plt.savefig('Kernel_compare_7-2.png') 
