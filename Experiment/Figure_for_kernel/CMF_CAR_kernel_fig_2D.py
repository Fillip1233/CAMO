...
def derive_forward(x1, x2):
    ...
    log_length_scales = torch.tensor([2])
    b = torch.tensor([1])
...
def derive_forward_2(x1, x2):
    ...
    log_length_scales = torch.tensor([1.2])
    b = torch.tensor([1])
    v = log_length_scales.exp() * b * 0.5
...
Sigma_derive_2 = derive_forward_2(x_test, x_test)
...
Z_derive_2 = Sigma_derive_2.detach().numpy()

# 绘制曲面图
fig = plt.figure(figsize=(10, 7))
ax = fig.add_subplot(111, projection='3d')
surf_1 = ax.plot_surface(X, Y, Z_ard, cmap='viridis', edgecolor='none', label = 'ARD')
surf_2 = ax.plot_surface(X, Y, Z_derive_2, cmap='plasma', edgecolor='none', label = 'Derive')
